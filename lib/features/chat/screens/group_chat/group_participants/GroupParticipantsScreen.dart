...
class _ParticipantsScreenState extends State<ParticipantsScreen> {
  ...
  Future<void> _loadParticipants() async {
    try {
      ...
      setState(() {
        errorMessage = 'Failed to load participants: $e';
        isLoading = false;
      });
    }
  }
  ...
  Future<void> _deleteParticipant(String email) async {
    try {
      ...
      await FirebaseFirestore.instance
          .collection('groupChats')
          .doc(widget.chatId)
          .update({'participants': FieldValue.arrayRemove([email])});

      setState(() {
        participants.remove(email);
        if (isAdmin) admins.remove(email);
      });

      if (isAdmin) await _removeAdmin(email);

      if (admins.isEmpty) {
        if (participants.isNotEmpty) {
          await _assignNewAdmin(participants[0]);
        } else {
          await _deleteGroup();
        }
      }
    } catch (e) {
      setState(() => errorMessage = 'Failed to remove participant: $e');
    }
  }
  ...
  void _confirmDelete(String email, String username) {
    showDialog(
      context: context,
  ...
  Widget _buildParticipantTile(String email) {
        ...
        return Container(
          ...
          child: ListTile(
            ...
            trailing: isCurrentUser
                ...
                : GestureDetector(
                    onTap: () => _confirmDelete(email, username),
                    child: Container(
                      width: 34,
                      height: 34,
                      decoration: BoxDecoration(
